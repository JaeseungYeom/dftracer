...
void test_data_event_serialization() {
    ...
    size_t size = serializer->data(buffer, index, event_name, category,
                                   start_time, duration, metadata, process_id, thread_id);
...
void test_aggregated_serialization() {
    ...
    AggregatedKey key1("posix", "read", 1000000, 5000, 1, meta1, nullptr, nullptr);
    AggregatedKey key2("posix", "write", 1000000, 2000, 1, meta2, nullptr, nullptr);
    
    // AggregatedDataType is map<TimeResolution, unordered_map<AggregatedKey, AggregatedValues*>>
...
void test_multiple_events() {
    ...
    for (size_t i = 0; i < 3; ++i) {
        ...
        total_size += serializer->data(buffer + total_size, i, "open", "posix",
